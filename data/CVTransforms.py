...
class RandomCropResize(object):
    ...
    def __call__(self, img, label):
        if (random.random() < 0.5) and self.cw !=0:
            h, w = img.shape[:2]
...
class ToMultiTensor(object):
    ...
    def __call__(self, image, label):
        ...
        h, w = label.shape[:2]
        image = cv2.resize(image, (int(w), int(h)))
        label4 = cv2.resize(label, (int(w/4), int(h/4)), interpolation=cv2.INTER_NEAREST)
        label3 = cv2.resize(label, (int(w/3), int(h/3)), interpolation=cv2.INTER_NEAREST)
        label2 = cv2.resize(label, (int(w/2), int(h/2)), interpolation=cv2.INTER_NEAREST)
        label1 = cv2.resize(label, (int(w/1), int(h/1)), interpolation=cv2.INTER_NEAREST)


        image = image.transpose((2,0,1))

        image_tensor = torch.from_numpy(image).div(255)
        label_tensor4 =  torch.LongTensor(np.array(label4, dtype=np.int)) #torch.from_numpy(label)
        label_tensor3 =  torch.LongTensor(np.array(label3, dtype=np.int)) #torch.from_numpy(label)
        label_tensor2 =  torch.LongTensor(np.array(label2, dtype=np.int)) #torch.from_numpy(label)
        label_tensor1 =  torch.LongTensor(np.array(label1, dtype=np.int)) #torch.from_numpy(label)

        return [image_tensor, label_tensor4, label_tensor3, label_tensor2, label_tensor1 ]
